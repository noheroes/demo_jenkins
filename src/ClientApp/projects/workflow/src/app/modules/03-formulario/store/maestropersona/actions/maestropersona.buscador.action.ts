...
import { IBuscardorMaestroPersona, IFormMaestroPersona, IFormBuscardorMaestroPersona, TIPO_PERSONA } from '../maestropersona.store.interface';
import { MaestropersonaService } from '../../../service/maestropersona.service';
import { AppCurrentFlowStore, AppAudit } from '@lic/core';
export class MaestroPersonaBuscadorActions {
  constructor(
  ...
  asynDeleteMaestroPersona = (id: string): Observable<IFormMaestroPersona> => {
    this.fetchMaestroPersonaBegin();
    const state = this.getState();
  ...
  private fetchPageMaestroPersonaSucces = (items: any, total: number, pageRequest: IDataGridPageRequest) => {
    ...
    const elementosTotal = (items || []).filter(item => !item.esEliminado && item.tipoPersonaEnum == TIPO_PERSONA.DOCENTE);

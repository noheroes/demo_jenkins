...
import { IModalProgramaDocente, IFormProgramaDocente } from '../maestropersona.store.interface';
import { ModalProgramaDocente, FormProgramaDocente } from '../maestropersona.store.model';
...
export class ProgramaDocenteModalActions {
  constructor(
    private getState: () => IModalProgramaDocente,
    private setState: (newState: IModalProgramaDocente) => void,
  ...
  resetModal = () => {
    ...
    this.setState(new ModalProgramaDocente());
  }
  ...
  asyncFetchCombos = (enums) => {
    /*debugger;
      ...
      let list=[];
      enums[0].filter(item => !item.esEliminado).map(element=>{
        if(this.getState().source.items.findIndex(item=>item.id==element.id)==-1)
        {
          list.push({
    ...
    */
   //console.log('CAYL asyncFetchCombos', enums[0]);
    const newState = {
      ...this.getState(),
      comboLists: {
      ...
      map(response => {
        ...
        programas.forEach(elementPrograma => {
          response.personas.filter(item => item.id == codigoPersona).forEach(element => {
            if (element.docente.programas.findIndex(itemPrograma => itemPrograma.idPrograma == elementPrograma.value && (!itemPrograma.esEliminado)) == -1)
      ...
      .pipe(
        map(response => {
          ...
          if (index != -1) {
            ...
            const Programas = response.personas[index].docente.programas || [];
      ...
      .pipe(
        map(response => {
          ...
          if (index != -1) {
            ...
            let persona = response.personas[index];
            const Programas = persona.docente.programas || [];
  ...
  asyncFetchPage = (
    ...
    return this.maestropersonaService.getFormatoByVersion(state.idVersion).pipe(
      tap(response => {
        ...
        this.fetchPageSucces(response, persona.docente.programas, response.count, pageRequest);
      }),
      catchError(error => {
        this.crudError(error);
        return throwError(error);
  ...
  private fetchPageSucces = (solicitud: any, items: any, total: number, pageRequest: IDataGridPageRequest) => {
    ...
    let elementos = elementosTotal.slice((pageRequest.page - 1) * pageRequest.pageSize, pageRequest.page * pageRequest.pageSize);

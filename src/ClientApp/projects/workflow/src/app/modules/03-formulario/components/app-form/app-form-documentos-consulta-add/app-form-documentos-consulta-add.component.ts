...
import { IModalDocumentosConsulta } from '../../../store/documentos-consulta/documentos-consulta.store.interface';
...
@Component({
    selector: 'app-form-documentos-consulta-add',
    templateUrl: './app-form-documentos-consulta-add.component.html',
    styleUrls: ['./app-form-documentos-consulta-add.component.css']
  })
export class AppFormDocumentoConsultaAddComponent implements OnInit{
    formType = FormType;
    form: FormGroup;
    store: DocumentosConsultaStore;
    state$: Observable<IModalDocumentosConsulta>;
    ...
    constructor(
        public dialogRef: MatDialogRef<AppFormDocumentoConsultaAddComponent>,
    ...
    ngOnInit() {
        this.state$ = this.store.state$.pipe(
            map((x) => x.documentosConsultaModal),
    ...
    private buildForms = () => {
        const { form, type, tipo } = this.store.state.documentosConsultaModal;
    ...
    const subs1 = this.store.state$
        .pipe(
        map((x) => x.documentosConsultaModal.form),
        distinctUntilChanged()
        )
        .subscribe((x) => {
    ...
    handleSubmit = () => {
        const { type } = this.store.state.documentosConsultaModal;
        ...
        .then((confirm) => {
          if (confirm) {
            ...
            //   this.store.documentosConsultaModalActions.asyncFetchRegistrarRTD(parametros)
            //   .subscribe((response) =>{
            //     if (response.success) {
    ...
    getParametros():any {
      const current = this.storeCurrent.currentFlowAction.get();
      const { tipo } = this.store.state.documentosConsultaModal;
    ...
    onPlantilla=async ()=>{
      const { tipo } = this.store.state.documentosConsultaModal;
      //const respuesta:any = await this.store.documentosOperacionActions.asyncDownLoadPlantilla(tipo.value); /
